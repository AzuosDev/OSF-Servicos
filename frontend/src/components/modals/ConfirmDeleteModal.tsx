...
export function ConfirmDeleteModal({ open, onClose, onDeleteOne, onDeleteGroup, accountName, isParcel, parcelLabel }: ConfirmDeleteModalProps) {
  return (
    <ModalShell open={open} title="Confirmar exclusão" icon={<AlertTriangle className="h-6 w-6 text-accent-red" />} onClose={onClose}>
      <div className="flex flex-col items-center space-y-4">
        ...
        <h2 className="text-lg font-bold text-white">{isParcel ? `Excluir ${parcelLabel ?? 'parcela'}?` : 'Excluir conta'}</h2>
